...
| `transcribe-call` | POST | 통화 오디오 → 다글로 STT/요약 → `consultations` 자동 기록 (§7) |
...
## 7. 상담 녹취 STT·요약 자동화 (다글로 연동)

`상담내역` 화면의 목표는 **통화 녹취 → 텍스트 변환 → 요약 → `consultations` 자동 기록**이다. 현재(1차)는 사람이 다글로 결과를 복사·붙여넣는 반자동이고, 2차 백엔드에서 API로 연결하면 붙여넣기까지 무인화된다.

### 단계별 자동화 수준

| 수준 | 방식 | 상태 |
|---|---|---|
| L0 반자동(현재) | 갤럭시 자동녹음 → 다글로 앱 변환 → 사람이 ERP에 붙여넣기 | 1차 완료 |
| L1 업로드 자동 | ERP에서 오디오 업로드 → 백엔드가 STT/요약 호출 → `consultations` 자동 insert | 2차 |
| L2 완전 무인 | 녹음이 클라우드에 쌓이면 백엔드가 폴링/웹훅으로 자동 처리 | 2차+ |

### L1 아키텍처 (권장)

```text
[직원 PC: 상담내역 → 오디오 업로드]
   │  (또는 폰 녹음폴더 → Drive → 백엔드 폴링)
   ▼
[Supabase Edge Function: transcribe-call]
   ├─ 오디오를 STT 업체에 전송 (다글로 API)
   ├─ 응답: 전체 텍스트(화자분리) + 요약
   ├─ consultations insert (channel=phone, content=요약, transcript=전문, recording=메타)
   └─ 고객 매칭: 통화 발신번호 ↔ customers.phone 자동 연결(없으면 미배정)
   ▼
[Realtime] 상담관리·고객 상담이력에 즉시 반영
```

### STT 업체 비교 (2026-07 기준, 정확한 단가는 계약 시 재확인)

| 업체 | 한국어 | 화자분리 | 요약 | 형태 |
|---|---|---|---|---|
| **다글로 (Daglo, ActionPower)** | 우수 | 지원 | 지원 | REST API + 콘솔, 국내 결제 |
| 네이버 CLOVA Speech | 우수 | 지원 | 별도 | NCP 내 통합 |
| Google Speech-to-Text | 양호 | 지원 | 별도(LLM) | 글로벌, 종량 |
| Whisper 自체호스팅 | 양호 | 별도 | 별도(LLM) | 서버 필요, 건당 비용 0 |

굿투어는 이미 다글로를 도입하므로 **다글로 API**를 그대로 승격해 쓰는 것이 학습비용이 낮다. 요약 품질을 더 높이려면 STT는 다글로, 요약은 Claude 등 LLM으로 2단 구성해도 된다.

### 다글로 API 연동 요점

- 다글로 **콘솔에서 API Key 발급** → Edge Function 시크릿으로만 보관(프런트 노출 금지)
- 호출: 오디오 파일(URL 또는 멀티파트) 전송 → job 생성 → 완료 시 결과(텍스트·화자·요약) 수신 (동기/비동기 폴링 또는 웹훅)
- 저장: 원본 오디오는 **Supabase Storage**(비공개 버킷), 텍스트·요약은 `consultations.transcript`/`content`
- 실패·부분인식 대비: job 상태를 `sms_logs`처럼 로그로 남기고 재시도 버튼 제공

### 개인정보 (중요)

- 통화 녹음·전사·요약은 **고객 동의 범위** 내에서만. 녹취 저장 시 동의 근거를 함께 보관
- 원본 오디오·전문은 보유기간(여행 종료 후 1년) 경과 시 배치 삭제(pg_cron)
- 외부 STT 전송은 **개인정보 처리위탁**에 해당 → 위탁 사실 고지 및 수탁사(다글로 등) 계약 필요

### 개발 범위 / 공수

| 작업 | 공수(대략) |
|---|---|
| Storage 버킷 + 업로드 UI(오디오 첨부 실저장) | 1일 |
| `transcribe-call` Edge Function (다글로 호출·폴링·저장) | 2~3일 |
| 발신번호↔고객 자동매칭, 실패 재시도, 상태 로그 | 1~2일 |
| L2 무인(클라우드 폴더 폴링/웹훅) | +2일 |

### 비용(월, 예시)

- 다글로 API: **종량제**(변환 시간 비례). 상담 통화가 하루 20건·평균 5분이면 월 약 50시간 변환 → 요금제 확인 필요
- Storage: 오디오 보관 용량 비례(수 GB 수준이면 Supabase Pro 내 포함)
- 요약을 LLM으로 별도 처리 시 토큰 비용 소액 추가
...
| 녹취 STT(다글로) | 선택 · 변환시간 종량 (반자동 L0은 0원) | 통화량 비례 |
| **합계** | **~2만원 미만** (STT 미사용 시) | **~6만원 + STT 실비** |
...
| 6주 | 실데이터 이전, 오픈 | 운영 전환 |
| 선택(+3~5일) | 상담 녹취 STT 자동화(§7): Storage+업로드, transcribe-call, 고객매칭 | 녹취 자동 기록 |

STT 자동화(§7)는 핵심 오픈 이후 부가 워크스트림으로 붙이는 것을 권장한다(오픈 지연 방지).
...
- 다글로 API Key (상담 녹취 STT 자동화 시 · §7) + 통화 녹음 개인정보 동의문
